...
public class Ball : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            _targetLinePoints[i] = Instantiate(TargetLinePoint, transform.position, Quaternion.identity);
        }
...
//    }
    ...
    private void LoadSprites(int id)
    {
        ...
        _sprite = Resources.Load<Sprite>("gfx/Skins/ball" + (id + 1));
    }
...
//        Defs.PlaySound(_ballRespown);
    ...
    void Update()
    {
        ...
        {
            if (!_isSetStartPoint)
//                &&((DefsGame.CurrentScreen == DefsGame.SCREEN_GAME)||(DefsGame.CurrentScreen == DefsGame.SCREEN_MENU)))
                SetStartPoint();
    ...
    private void DrawTargetLine()
    {
        if (_isSetStartPoint)
            if (InputController.IsTouchOnScreen(TouchPhase.Moved))
            {
                ...
                _mousePosition = Camera.main.ScreenToWorldPoint(Input.mousePosition);
                D.Log(_mousePosition);

                _mouseTarget.x += (_mousePosition.x - _oldMousePosition.x) * 2f;
                _mouseTarget.y += (_mousePosition.y - _oldMousePosition.y) * 2f;

//                if (_mouseTarget.y > ScreenOptions.ScreenHeight * 0.5f)
//                {
//                    _mouseTarget.y = ScreenOptions.ScreenHeight * 0.5f;
//                }
            }
...
//        else if (other.gameObject.CompareTag("ShieldTrigger"))
//        {
//            _isShield = true;
...
//        }
//        else if (other.gameObject.CompareTag("ShieldTopTrigger"))
//        {
//            _isShield = true;
////            Defs.PlaySound(_ballVsGround, Mathf.Min(_body.velocity.sqrMagnitude, 150f)/150f);
//            Instantiate(BallVsGroundAnimator,
...
//                Quaternion.identity);
//        }
//        else if (other.gameObject.CompareTag("RingTrigger"))
//        {
//            _isRing = true;
////            Defs.PlaySound(_ballVsRing, Mathf.Min(_body.velocity.sqrMagnitude, 100f)/100f);
//        }
//        else if (other.gameObject.CompareTag("WebTrigger"))
//        {
//            if (!_isWeb)
